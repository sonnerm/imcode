...
def heisenberg_T(t,Jx,Jy,Jz,hx,hy,hz,Jxe=None,Jye=None,Jze=None,init=np.eye(4),final=np.eye(4)):
    if Jxe is None:
        Jxe=Jx
    if Jye is None:
        Jye=Jy
    if Jze is None:
        Jze=Jz
    return brickwork_T(t,heisenberg_gate(Jxe,Jye,Jze),heisenberg_gate(Jx,Jy,Jz,hx,hy,hz),init,final)
def heisenberg_La(t):
