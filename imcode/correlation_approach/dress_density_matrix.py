...
def dress_density_matrix(rho_0_exponent, F_E_prime, F_E_prime_dagger,M_E, nbr_Floquet_layers,init_state):
    nsites = int (len(rho_0_exponent[0]) / 2)

    rho_0_single_body = expm(- rho_0_exponent) @ expm(- rho_0_exponent)
    if init_state == 2:
