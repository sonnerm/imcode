...
def dress_density_matrix(rho_0, F_E_prime, F_E_prime_dagger, nbr_Floquet_layers):
    ...
    if linalg.norm(rho_dressed - identity(len(rho_0[0]))) > 1e-10: #else, eigenvalues are zero and N_t is identity
    #diagonalize dressed density matrix:
        random_part = np.random.rand(len(rho_0[0]),len(rho_0[0])) * 1e-14
        # symmetrize random part
        for i in range(len(rho_0[0])):
            for j in range(i, len(rho_0[0])):
                random_part[i, j] = random_part[j, i]
        #rho_dressed += random_part
