...
def evolvers(M_fw, M_fw_inverse, M_bw, M_bw_inverse,  N_t, eigenvalues_G_eff_fw, eigenvalues_G_eff_bw, nsites, ntimes, beta_tilde):
    ...
    for tau in range(0, ntimes):
        T_tilde[0, tau] = np.einsum('ij,jk,kl,lm,mn->in', D_beta_fw, M_fw,
                                    matrix_power(D_phi_fw, tau + 1), M_bw_inverse, N_t)  # forward branch
        T_tilde[1, tau] = np.einsum('ij,jk,kl,lm,mn->in', D_beta_bw, M_bw,
                                    matrix_power(D_phi_bw, tau + 1), M_bw_inverse, N_t)  # backward branch
