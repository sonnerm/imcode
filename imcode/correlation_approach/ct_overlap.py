...
with h5py.File(filename+'_DMs_ct' + ".hdf5", 'w') as f:
    dset_DM = f.create_dataset('density_matrix', (max_time,4,4), dtype=np.complex_)

with h5py.File(filename+'_spinfulpropag_ct' + ".hdf5", 'w') as f:
...
for iter in range(1,2,interval):
    ...
    #rotate into correct basis with input/output variables
        ...
        for i in range(0,B.shape[0] //4, 2):
            rot[i,i] = 1./np.sqrt(2)
            rot[i,i+1] = 1./np.sqrt(2)
            rot[i+1,i] = - 1./np.sqrt(2) * np.sign(B.shape[0] //2 - i-1)
            rot[i+1,i+1] = 1./np.sqrt(2) * np.sign(B.shape[0] //2 - i-1)
        B = rot.T @ S @ B @ S.T @ rot 
    ...
    # Influence matrices for both spin species
    #spin down
    exponent[dim_B:2*dim_B,dim_B:2*dim_B] = B_enlarged_inv[:,:]
    #spin up 
    exponent[2*dim_B:3*dim_B, 2*dim_B:3*dim_B] = B_enlarged_inv[:,:]
    
    exponent *= 0.5 # to avoid double couting from next antisymmetrization
    # integration measure
    # spin up
    ...
    for i in range(dim_B//4-1):
        ...
        exponent[dim_B//2 - 3 - 2*i, dim_B//2 - 2 - 2*i] += 1+1.j * mu_up  #1 *np.cos(t) *np.exp(1.j * mu_up) 
        #(matrix elements between down -> down) 
        exponent[3*dim_B + dim_B//2 - 3 - 2*i, 3*dim_B + dim_B//2 - 2 - 2*i] += 1+1.j * mu_down #1 *np.cos(t) *np.exp(1.j * mu_down) 
        ...
        exponent[dim_B//2 + 1 + 2*i, dim_B//2 + 2 + 2*i] += 1-1.j * mu_up #1 *np.cos(t) * np.exp(-1.j * mu_up) 
        exponent[3*dim_B + dim_B//2 + 1 + 2*i, 3*dim_B + dim_B//2 + 2 + 2*i] += 1 -1.j * mu_down #1 *np.cos(t) * np.exp(-1.j * mu_down) 
    ...
    exponent -= exponent.T #matrix is antisymmetric
    ...
    # temporal boundary condition from overlap
    ...
    N = np.sqrt(linalg.det(B))
    for iter in range (2,200):
        #times[iter] = iter
        delt = 2 * (total_time - iter)
        with h5py.File(filename+'_spinfulpropag_ct' + ".hdf5", 'a') as f:
            ...
            propag_data[iter] = 1-exponent_inv[2*dim_B + dim_B//2 -2*iter -2 ,2*dim_B + dim_B//2 -2*iter -1]  #<n(t)># works
    ...
    print(times)
    print(propag)
    print('adf',np.sqrt(linalg.det(B)))
