...
def plot_entropy(entropy_values, iterator, Jx, Jy, g, del_t,beta, nsites, method,  ising_gamma_times= [0], gamma_test_vals=[0]):
    # plot
    ...
    np.savetxt(work_path + method + 'ent_entropy_Jx=' + str(Jx) + '_Jy=' + str(Jy) + '_g=' + str(g) +'_del_t=' + str(del_t)+ '_beta=' + str(beta) + '_L=' + str(nsites) + '.txt', entropy_values,  delimiter=' ', fmt='%1.5f')


    plt.savefig(work_path + method + 'ent_entropy_Jx=' + str(Jx) + '_Jy=' + str(Jy) + '_g=' +
                str(g) +'_del_t=' + str(del_t) + '_beta=' + str(beta)+ '_L=' + str(nsites) + '.png')
