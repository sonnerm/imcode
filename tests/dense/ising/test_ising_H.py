...
def test_ising_H(seed_rng):
    ...
    assert (ret - dense.sz(L, 0) @ dense.sz(L, L - 1) * J[-1]) == pytest.approx(dense.ising.ising_H(J[:-1], g, h))
    ...
    assert (dense.ising.ising_H(J, g, h) == dense.ising.ising_H(J, g, h).T).all()
def test_single_site_ising_H():
    ...
    diH=dense.ising.ising_H([],g,h)
    assert diH==pytest.approx(dense.SZ*h+dense.SX*g)
    diH=dense.ising.ising_H([0.0],g,h)
    assert diH==pytest.approx(dense.SZ*h+dense.SX*g)
