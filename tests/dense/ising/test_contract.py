...
def test_contract_1x3_mixed(seed_rng):
    L=1
    t=3
    init=np.random.normal(size=(2,2))+1.0j*np.random.normal((2,2))
    final=np.random.normal(size=(2,2))+1.0j*np.random.normal((2,2))
    g=np.random.normal()
    h=np.random.normal()
...
def test_contract_3x3_mixed(seed_rng):
    ...
    assert transverse==pytest.approx(direct)
# def test_contract_3x3_periodic(seed_rng):
#     L=3
#     t=3
#     init=[np.random.normal(size=(2,2))+1.0j*np.random.normal((2,2)) for _ in range(L)]
#     final=[np.random.normal(size=(2,2))+1.0j*np.random.normal((2,2)) for _ in range(L)]
...
#     initv=dense.kron(init)
#     finalv=dense.kron(final)
#     transverse=np.trace(reduce(lambda x,y:x@y,Ts))
#     for _ in range(t):
#         initv=F@initv@F.T.conj()
#     direct=np.trace(finalv@initv)
#     assert transverse==pytest.approx(direct)
...
#     L=3
#     t=3
#     init=[np.random.normal(size=(2,))+1.0j*np.random.normal((2,)) for _ in range(L)]
#     final=[np.random.normal(size=(2,))+1.0j*np.random.normal((2,)) for _ in range(L)]
#     init=[i/np.sqrt(i.conj()@i) for i in init]
#     final=[f/np.sqrt(f.conj()@f) for f in final]
#     Js=np.random.normal(size=(L-1,))
#     gs=np.random.normal(size=(L,))
#     hs=np.random.normal(size=(L,))
#     Ts=[dense.ising.ising_T(t,J,g,h,i,f) for J,g,h,i,f in zip(Js,gs[:-1],hs[:-1],init[:-1],final[:-1])]
...
#     F=dense.ising.ising_F(L,Js,gs,hs)
#     bc=dense.ising.open_boundary_im(t)
#     initv=dense.outer(init)
#     finalv=dense.outer(final)
#     for T in Ts:
#         bc=T@bc
#     transverse=dense.ising.open_boundary_im(t)@bc
#     for _ in range(t):
