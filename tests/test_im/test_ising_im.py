...
def test_mps_ising_im_iterative(dense_ising_im):
    assert mps.mps_to_dense(mps.im_iterative(mps.ising_T(*dense_ising_im[1])))==pytest.approx(dense_ising_im[0])
@pytest.mark.xfail
def test_mps_ising_im_dmrg(dense_ising_im):
    assert mps.mps_to_dense(mps.im_dmrg(mps.ising_T(*dense_ising_im[1])))==pytest.approx(dense_ising_im[0])
